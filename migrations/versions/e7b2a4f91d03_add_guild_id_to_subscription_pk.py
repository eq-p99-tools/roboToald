...
_existing_table = sa.Table(
    ...
    sa.PrimaryKeyConstraint("user_id", "target", name="pk_user_id_target"),
)


def upgrade() -> None:
    """Upgrade schema."""
    op.execute(f"UPDATE subscriptions SET guild_id = {LEGACY_GUILD_ID} WHERE guild_id IS NULL OR guild_id = 0")

    with op.batch_alter_table("subscriptions", copy_from=_existing_table, recreate="always") as batch_op:
        batch_op.drop_constraint("pk_user_id_target", type_="primary")
        batch_op.create_primary_key("pk_user_target_guild", ["user_id", "target", "guild_id"])
...
def downgrade() -> None:
    ...
    _new_table = sa.Table(
        ...
        sa.Column("last_window_start", sa.Integer, default=0),
        sa.Column("guild_id", sa.Integer, nullable=False),
        sa.PrimaryKeyConstraint("user_id", "target", "guild_id", name="pk_user_target_guild"),
    )

    with op.batch_alter_table("subscriptions", copy_from=_new_table, recreate="always") as batch_op:
        batch_op.drop_constraint("pk_user_target_guild", type_="primary")
        batch_op.create_primary_key("pk_user_id_target", ["user_id", "target"])
