...
class Subscription(base.Base, base.MyBase):
    ...
    expiry = sqlalchemy.Column(sqlalchemy.Integer)
    last_notified = sqlalchemy.Column(sqlalchemy.Integer)
    ...
    def __init__(self, user_id, target, expiry, last_notified, guild_id):
...
def get_subscriptions_for_notification() -> List[Subscription]:
    with base.get_session() as session:
        subs = session.query(Subscription).filter(
            Subscription.last_notified < time.time() - 31 * 60).all()
    return subs


def mark_subscription_sent(user_id: int, target: str) -> bool:
    with base.get_session() as session:
