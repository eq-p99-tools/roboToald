...
def _hmac_sign(data: bytes) -> str:
    return hmac.new(config.ENCRYPTION_KEY.encode(), data, hashlib.sha256).hexdigest()
...
def _sso_guild_ids(
    ...
    ids = [gid for gid in session_guilds if config.GUILD_SETTINGS.get(gid, {}).get("enable_sso")]
        ...
        entries.append(
            {
                ...
                "user_name": (
                    ...
                    if log.discord_user_id and log.guild_id
                    else ""
                ),
...
async def partial_rate_limited(request: Request):
    ...
    is_super = session.get("super", False)
    rate_limited = sso_model.get_rate_limited_ips(config.RATE_LIMIT_MAX_ATTEMPTS, config.RATE_LIMIT_WINDOW_MINUTES)
    hashed = [(sso_model.ip_country_code(ip), sso_model.hash_ip(ip), count) for ip, count in rate_limited]

    return templates.TemplateResponse(
        request,
        "partials/rate_limited.html",
        {
            "rate_limited": hashed,
            "is_super": is_super,
        },
